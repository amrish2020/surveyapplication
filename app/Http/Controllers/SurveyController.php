...
class SurveyController extends Controller
{
    ...
    public function getIndex()
    {
        $user = Auth::user();
        $survey = new Survey();
        $surveys = $survey->getSurveyResults($user->id);
        $questions = Question::orderBy('created_at', 'asc')->get();
    ...
    public function surveyCreate(Request $request)
    {
        ...
        {
            DB::insert('insert into survey_results_detail(survey_results_id, question_id,answer_id,created_at,updated_at) values (?,?,?,?,?)', [$last_insert_id,$key, $value,date('Y-m-d h:i:s'),date('Y-m-d h:i:s')]);
        }
    ...
    public function surveyResults($id)
     ...
     public function getAdminSurveyReport($id){
        $survey = new Survey();
         $surveys[] = $survey->getSurvey($id);
         $survey_name = $surveys[0]["survey_name"];
         $survey_id = $surveys[0]["id"];
         $survey_results = $survey->getResultsReport($id);

         return view('admin.surveys.report', ['surveys' => $surveys, 'survey_name' => $survey_name, 'survey_results' => $survey_results, 'survey_id' => $survey_id]);
